...
public class Input {
    ...
    public static MenuOption inputMenuOption() {
        ...
        String input = inputDialog(output, MenuOption.ERROR.getTitle(), s -> validateMenuOption(s));
    ...
    public static String inputRussianLetter() {
        ...
        return inputDialog(output, "Вы ввели что-то другое! Букву на русском языке!", s -> validateLetters(s) && validateLength(s));
    }
    ...
    private static boolean validateMenuOption(String word) {
        return (word.equals(MenuOption.PLAY.getTitle().toLowerCase()) || word.equals(MenuOption.EXIT.getTitle().toLowerCase()));
    }
